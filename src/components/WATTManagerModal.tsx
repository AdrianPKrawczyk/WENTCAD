...
export function WATTManagerModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  const [partitionSubTab, setPartitionSubTab] = useState<'EXT_WALL' | 'INT_WALL' | 'FLOOR' | 'ROOF' | 'WINDOW' | 'DOOR'>('EXT_WALL');
  const [partitionViewMode, setPartitionViewMode] = useState<'GRID' | 'TABLE'>('GRID');
  const [isOpeningStyleModalOpen, setIsOpeningStyleModalOpen] = useState(false);
  const [editingOpeningStyle, setEditingOpeningStyle] = useState<IfcWindowStyle | null>(null);
  ...
  return (
    <div className="fixed inset-0 z-[1000] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-5xl overflow-hidden flex flex-col max-h-[90vh]">
        ...
        {/* CSS for custom scrollbars if needed - keeping it light */}
          ...
          {activeTab === 'WALL_TYPES' && (
            <div className="space-y-6">
               <div className="bg-blue-50 border border-blue-100 rounded-xl p-4 text-blue-800 text-xs flex gap-3 items-center">
                  ...
                  <p>Typy przegród i stolarki pozwalają na definiowanie standardów termicznych dla projektu. Współczynnik <b>U</b> dla przegród wielowarstwowych jest wyliczany automatycznie.</p>
               </div>

               {/* Partition Sub-tabs and View Switcher */}
               <div className="flex items-center gap-4 border-b border-gray-100 mb-6 bg-gray-50/50 p-1.5 rounded-2xl">
                 <div className="flex items-center gap-1 overflow-x-auto scrollbar-hide flex-1">
                   {[
                     { id: 'EXT_WALL', label: 'Ściany Zewn.' },
                     { id: 'INT_WALL', label: 'Ściany Wewn.' },
                     { id: 'ROOF', label: 'Stropy / Dachy' },
                     { id: 'FLOOR', label: 'Podłogi' },
                     { id: 'WINDOW', label: 'Okna' },
                     { id: 'DOOR', label: 'Drzwi' }
                   ].map(sub => (
                     <button
                       key={sub.id}
                       onClick={() => setPartitionSubTab(sub.id as any)}
                       className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all shrink-0 ${
                         partitionSubTab === sub.id 
                 ...
                 <div className="flex items-center gap-1 bg-white/50 p-1 rounded-xl border border-gray-100 shrink-0">
                   <button 
                     onClick={() => setPartitionViewMode('GRID')}
                     className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-[10px] font-bold uppercase tracking-tight transition-all ${
                       partitionViewMode === 'GRID' ? 'bg-indigo-600 text-white shadow-md' : 'text-gray-400 hover:text-gray-600'
                     }`}
                   >
                     <LayoutGrid className="w-3.5 h-3.5" />
                     <span>Siatka</span>
                   ...
                   <button 
                     onClick={() => setPartitionViewMode('TABLE')}
                     className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-[10px] font-bold uppercase tracking-tight transition-all ${
                       partitionViewMode === 'TABLE' ? 'bg-indigo-600 text-white shadow-md' : 'text-gray-400 hover:text-gray-600'
                     }`}
                   >
                     <List className="w-3.5 h-3.5" />
                     <span>Tabela</span>
               ...
               <div className="space-y-6">
                  {partitionSubTab !== 'WINDOW' && partitionSubTab !== 'DOOR' ? (
                    <>
                      {/* WALL TYPES GRID/TABLE */}
                      {partitionViewMode === 'GRID' ? (
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                          {Object.values(wallTypes)
                            .filter(wt => {
                              if (partitionSubTab === 'EXT_WALL') return wt.thermalType === 'WALL' && wt.isExternal;
                              if (partitionSubTab === 'INT_WALL') return wt.thermalType === 'WALL' && !wt.isExternal;
                              if (partitionSubTab === 'FLOOR') return wt.thermalType === 'FLOOR';
                              if (partitionSubTab === 'ROOF') return wt.thermalType === 'ROOF';
                              return false;
                            }).length === 0 ? (
                              <div className="col-span-full py-12 text-center text-gray-300 text-xs italic bg-gray-50 rounded-2xl border-2 border-dashed border-gray-100 text-sm">Brak przegród w tej kategorii.</div>
                            ) : (
                              Object.values(wallTypes)
                                .filter(wt => {
                                  if (partitionSubTab === 'EXT_WALL') return wt.thermalType === 'WALL' && wt.isExternal;
                                ...
                                .map(wt => (
                                  <div key={wt.id} className="group p-5 bg-white rounded-2xl border border-gray-100 hover:border-indigo-200 hover:shadow-xl hover:shadow-indigo-50/50 transition-all relative">
                                     ...
                                     <h4 className="font-bold text-gray-800 text-sm mb-1">{wt.name}</h4>
                                     <div className="flex items-center gap-2 text-[9px] font-bold text-gray-400 uppercase tracking-widest">
                                        <span>Standard: {wt.predefinedType}</span>
                                        {wt.isGroundContact && <span className="bg-amber-100 text-amber-600 px-1 rounded">Grunt</span>}
                                     </div>
                                  </div>
                                ))
                            )}
                          ...
                          <table className="w-full text-left text-xs">
                             ...
                             <tbody className="divide-y divide-gray-50 bg-white">
                                {Object.values(wallTypes)
                                  ...
                                  .map(wt => (
                                    <tr key={wt.id} className="hover:bg-indigo-50/30 transition-colors group">
                                       <td className="px-6 py-4 font-bold text-gray-700">{wt.name}</td>
                      ...
                      <div className="mt-8">
                        <button 
                          onClick={() => { setEditingWallType(null); setIsWallTypeModalOpen(true); }}
                        ...
                        >
                          <Plus className="w-5 h-5" /> Stwórz nową przegrodę warstwową
                        </button>
                      </div>
                    </>
                  ) : (
                    <>
                      {/* WINDOWS/DOORS LISTING */}
                      {partitionViewMode === 'GRID' ? (
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                          {Object.values(windowStyles)
                            .filter(ws => {
                              if (partitionSubTab === 'WINDOW') return ws.type === 'WINDOW' || !ws.type;
                              if (partitionSubTab === 'DOOR') return ws.type === 'DOOR';
                              return false;
                            }).length === 0 ? (
                              <div className="col-span-full py-12 text-center text-gray-300 text-xs italic bg-gray-50 rounded-2xl border-2 border-dashed border-gray-100 text-sm">Brak modeli stolarki w tej kategorii.</div>
                            ) : (
                              Object.values(windowStyles)
                                .filter(ws => {
                                  if (partitionSubTab === 'WINDOW') return ws.type === 'WINDOW' || !ws.type;
                                ...
                                .map(ws => (
                                  <div key={ws.id} className="group p-5 bg-white rounded-2xl border border-gray-100 hover:border-sky-300 hover:shadow-xl hover:shadow-sky-50 transition-all relative">
                                     <div className="flex justify-between items-start mb-4">
                                        <div className={`w-10 h-10 rounded-xl flex items-center justify-center font-black text-xs border ${partitionSubTab === 'WINDOW' ? 'bg-sky-50 text-sky-600 border-sky-100' : 'bg-emerald-50 text-emerald-600 border-emerald-100'}`}>
                                            {partitionSubTab === 'WINDOW' ? 'WIN' : 'DOR'}
                                        </div>
                                        <div className="flex gap-1">
                                          <button 
                                            onClick={() => { setEditingOpeningStyle(ws); setIsOpeningStyleModalOpen(true); }}
                                            className="p-1.5 text-gray-300 hover:text-sky-600 transition-colors"
                                          ><Edit2 className="w-4 h-4" /></button>
                                          <button 
                                            onClick={() => removeWindowStyle(ws.id)}
                                            className="p-1.5 text-gray-300 hover:text-red-500 transition-colors"
                                          ><Trash2 className="w-4 h-4" /></button>
                                        </div>
                                     </div>
                                     <h4 className="font-bold text-gray-800 text-sm mb-1">{ws.name}</h4>
                                     <div className="space-y-1">
                                        <div className="flex justify-between text-[10px] uppercase font-bold text-gray-400">
                                          <span>U-Value</span>
                                          <span className="text-gray-900">{ws.overallUValue} W/m²K</span>
                                        </div>
                                        <div className="flex justify-between text-[10px] uppercase font-bold text-gray-400">
                                          <span>g-Value (SHGC)</span>
                                          <span className="text-gray-900">{ws.solarHeatGainCoefficient}</span>
                                        </div>
                                     </div>
                                  </div>
                                ))
                        ...
                        <div className="border border-gray-100 rounded-2xl overflow-hidden shadow-sm">
                           <table className="w-full text-left text-xs">
                             <thead className="bg-gray-50 border-b border-gray-100 text-[10px] uppercase font-bold text-gray-400 tracking-wider">
                                <tr>
                                   <th className="px-6 py-4">Model Stolarki</th>
                                   <th className="px-6 py-4 text-center">U [W/m²K]</th>
                             ...
                             <tbody className="divide-y divide-gray-50 bg-white">
                                {Object.values(windowStyles)
                                  ...
                                  .map(ws => (
                                    <tr key={ws.id} className="hover:bg-sky-50/30 transition-colors group">
                                       <td className="px-6 py-4 font-bold text-gray-700">{ws.name}</td>
                                       <td className="px-6 py-4 text-center font-mono font-bold text-sky-800">{ws.overallUValue}</td>
                                       <td className="px-6 py-4 text-center font-mono text-gray-500">{ws.solarHeatGainCoefficient}</td>
                                       <td className="px-6 py-4 text-right">
                                          <div className="flex justify-end gap-2">
                                             <button 
                                              onClick={() => { setEditingOpeningStyle(ws); setIsOpeningStyleModalOpen(true); }}
                      ...
                      <div className="mt-8">
                        <button 
                          onClick={() => { setEditingOpeningStyle(null); setIsOpeningStyleModalOpen(true); }}
                          className={`w-full py-4 border-2 border-dashed rounded-2xl font-bold transition-all flex items-center justify-center gap-2 ${partitionSubTab === 'WINDOW' ? 'border-sky-200 text-sky-600 hover:bg-sky-50' : 'border-emerald-200 text-emerald-600 hover:bg-emerald-50'}`}
                        >
                          <Plus className="w-5 h-5" /> Dodaj nowy styl {partitionSubTab === 'WINDOW' ? 'okna' : 'drzwi'}
                        </button>
                      </div>
                    </>
                  )}
               ...
               {/* GLOBAL TEMPLATES SECTION (Wall Types Only) */}
               {(partitionSubTab !== 'WINDOW' && partitionSubTab !== 'DOOR') && wallTypeTemplates.length > 0 && (
        ...
        <OpeningStyleModal 
          isOpen={isOpeningStyleModalOpen}
          editingStyle={editingOpeningStyle || undefined}
          defaultType={partitionSubTab === 'DOOR' ? 'DOOR' : 'WINDOW'}
          onClose={() => {
            setIsOpeningStyleModalOpen(false);
            setEditingOpeningStyle(null);
          }}
        />
      </div>
    </div>
